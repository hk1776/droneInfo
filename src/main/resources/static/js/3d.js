...
loader.load('../images/knx2.obj', function (loadObject) {
    object = loadObject;
    scene.add(object);
    object.position.set(-1.8,0,-1.9); // 원하는 pivot point 좌표로 설정
...
function animate(x,y,z) {
    ...
    requestAnimationFrame(animate);     console.log("애니메이션")
...
//트리 각도 변경 함수
function rotateObjectBasedOnCondition(x,y,z) {
    if(object){
        object.rotation.x = x; // 원하는 각도로 X축 회전
        object.rotation.y = y; // 원하는 각도로 Y축 회전
        object.rotation.z = z;
    }
}
